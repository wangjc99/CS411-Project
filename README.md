...
With client information, I will be able to get users' playlists. Here, it will store user's playlist to MongoDB.
```Python
MONGO_URI='mongodb://dbUser:Password1@ds031108.mlab.com:31108/csproj?retryWrites=false'
```

 However, Spotify is well-known for not providing lyrics. I found  [MusixMatch](https://developer.musixmatch.com/documentation/api-reference/) API that is super handy. An example usage is below.
 ```Python
...
- Python Flask vs. Node JS
    - I admit that I went for a non-industry but more handy tool Python. Although Python Flask was very easy to set up, but I cannot find enough material online to deal with OAuth and deployment.
    - OAuth requires cross-domain requests, I did not find out how to deal with it and thus our OAuth works only if I enter this in our browser `https://accounts.spotify.com/authorize?client_id=a5ed2648aaed434d8f4fbe45d30a5cb6&redirect_uri=http://localhost:5000&response_type=token`
    - Deployment: I am literally learning by doing... I traced down to `gunicorn` an add-on for `Flask`... I tried to upload files to _PythonAnywhere_ and _Heroku_ but I did not get it to work

- Module spotipy vs. Spotify OAuth
    - I spent some time on spotipy, a wrapper for Spotify APIs. Then I learned, I needed to process Client-side OAuth, not Sever-side.
    - When I tried client-side OAuth, I ran into CORS trouble... I tried Jsonp and adding headers; neither worked.

- Google API, Twitter API
    - I tried.
    - Google API obviously has better documentations, but it's restrictions are tight. Especially with Google API, it requires API key and OAuth2 at the same time. i had a hard time setting up OAuth2 redirect_URI and decided to abandon it.
